...
//			conn.close();
	...
	public ObservableList<principal> getPrincipals() {
		...
		try {
			...
			// Step 2
			sql = "SELECT P.id,D.department_name,CONCAT(E.last_name,' ',E.middle_name,' ',E.first_name),CONCAT(P.value_money,'$'),P.`type`,P.date_principal,P.created_at,P.description, P.employee_id FROM hrm.principal AS P "
					+ "INNER JOIN hrm.employee AS E ON P.employee_id=E.id "
					+ "INNER JOIN hrm.department AS D ON D.id=E.department_id  WHERE P.flag=0  \r\n" + "\r\n" + "";
			// Step 3
			PreparedStatement pst = conn.prepareStatement(sql);
			// Step 4
