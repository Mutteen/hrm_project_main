...
public class positionemployeeDAO implements DAO<position_employee> {
	...
	public ObservableList<position_employee> getList() {
		...
		try {
			...
			// Step 2
			sql = "SELECT P.id ,P.employee_id,P.position_id,P.description,P.created_at,CONCAT(E.last_name,' ',E.middle_name,' ',E.first_name), PO.position_name \r\n"
					+ "FROM hrm.position_employee AS P \r\n"
					+ "INNER JOIN hrm.employee AS E ON E.id =P.employee_id  \r\n"
					+ "INNER JOIN hrm.`position` AS PO ON PO.id=P.position_id WHERE P.flag=0";
