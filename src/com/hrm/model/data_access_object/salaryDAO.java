...
public class salaryDAO implements DAO<salary> {
	...
	public ObservableList<salary> getAll() {
		...
		try {
			...
			// Step 2
			sql = "SELECT S.id,S.employee_id,CONCAT(E.last_name,' ',E.middle_name,' ',E.first_name),CONCAT(S.value_money,'$'),S.time_to_pay,S.who_pay,CONCAT(S.value_money_reward,'$'),S.working_day,S.created_at,S.description,S.flag FROM hrm.salary AS S INNER JOIN hrm.employee AS E ON E.id=S.employee_id WHERE S.flag=0 ORDER  BY time_to_pay ASC"; // Step
																																																																																							// 3
			...
			while (rs.next()) {
				...
				sal.setSEmployee(rs.getString(3));
...
//			conn.close();
	...
	public boolean save(salary t) {
		...
		try {
			...
			sql = "INSERT INTO `hrm`.`salary` (`employee_id`, `value_money`, `time_to_pay`, `value_money_reward`, `working_day`, `created_at`,`who_pay`,`description`) VALUES (?, ?, ?, ?, ?, ?,?,?);";
			PreparedStatement pst = conn.prepareStatement(sql);
