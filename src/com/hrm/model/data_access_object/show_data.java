...
public class show_data<T> {
	...
	public ArrayList<T> getProfile(int idUser) throws SQLException {
		...
		String query = "SELECT employee.id, employee.last_name,employee.middle_name,employee.first_name,employee.username, employee.dob, employee.email, employee.telephone, employee.address, employee.on_leave, employee.description,employee.`status`, employee.hire_date,\r\n"
	...
	public ArrayList<T> getAllEmployee() throws SQLException {
		...
		String query = "SELECT employee.id ,employee.last_name,employee.middle_name,employee.first_name,employee.username ,employee.dob, employee.email, employee.telephone, employee.address, employee.on_leave, employee.description,employee.`status`, employee.hire_date,\r\n"
				...
				+ "INNER JOIN salary  ON salary.employee_id  = employee.id\r\n"
				+ "INNER JOIN value_money_principal ON value_money_principal.employee_id = employee.id\r\n"
				+ "WHERE MONTH(time_to_pay) = MONTH(CURRENT_TIMESTAMP())AND  employee.flag=0 "
				+ "GROUP BY employee.id;";
